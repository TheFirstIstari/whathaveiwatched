...
use crate::helpers::watch::{collect_leaf_ids, recompute_aggregates, upsert_watch_entry};
...
pub fn set_watch(ctx: &ReducerContext, board_id: u64, media_item_id: u64, watched: bool) -> Result<(), String> {
    ...
    recompute_aggregates(ctx, media_item_id, ctx.sender);
    Ok(())
}
...
pub fn reset_progress(ctx: &ReducerContext, board_id: u64) -> Result<(), String> {
    ...
    let entries: Vec<_> = ctx.db.watch_entry().iter()
        .filter(|e| e.board_id == board_id && e.watcher_identity == ctx.sender).collect();
    for row in entries { ctx.db.watch_entry().id().delete(row.id); }
    let aggs: Vec<_> = ctx.db.watch_aggregate().iter()
        .filter(|a| a.board_id == board_id && a.watcher_identity == ctx.sender).collect();
